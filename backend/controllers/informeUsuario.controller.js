...
const informeUsuario = db.informeUser;
...
exports.create = (req, res) =>{
    ...
    informeUsuario.create(informeUsuario)
    .then(data =>{
        res.send(data);
    })
    .catch(err => {
...
exports.update = (req, res) => {
    ...
    informeUsuario.update(req.body, {
        where: { idReport: id }
    })
        .then(num =>{
            if (num == 1) {
                res.send({
                    message: "El informeUsuario fue actualizado exitosamente."
                });
            } else {
                res.send({
